...
/*
	...
	The function takes the follwing arguments:
		...
		- batch_hiddens* hidden_states is an array of structs storing the hidden state directly prior to 
		                                the last layer for each example within the batch array.
		                              
		- int batch_size

*/
void backprop_W_2(weights* net, batch_outputs* batch, batch_hiddens* hidden_states, int batch_size)
{
	double gradient = 0;

	for (int b = 0; b < batch_size; b++)   // iterate through each example of the batch
	{
		for (int i = 0; i < net->W_2_rows; i++)  // iterate through rows of weight matrix
		{
			for (int j=0; j < net->W_2_cols; j++)  // iterate through cols of weight matrix 
			{
			
				gradient = (batch[b].output_vector[i] - batch[b].target) * hidden_states[b].hidden[j];  // compute gradient  -----  dL/dWij = (p_i - y_i) * h_j    
